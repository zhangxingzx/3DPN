...
void PLC()
{
	while(dianzulv::m_dianzulv->m_plcRun)
	{
		...
		if (Initialvalue[0] == 1)//判断硅片PN型
		{
			emit dianzulv::m_dianzulv->SignPN();   //发送信号
		}
		if (Initialvalue[1] == 1)//判断硅片电压信号是否接收
		{
			for (int i = 0; i < 200; i++)
			{
			...
			dianzulv::m_dianzulv->m_p.clear();
			dianzulv::m_dianzulv->m_p=dianzulv::m_dianzulv->find_P(dianzulv::m_dianzulv-> voltage);  //电阻率函数赋值
			cout << "123456789" << endl;
		} 
		//通过句柄写入结构体的值
...
void dianzulv::draw() //绘图
{
	...
	if (m_p.size()!= 0)
	{
		//ww = m_p[m_p.size() - 2];
		//cout << ww << ": " << endl;
		//QString strAvg_U = QString::number(ww, 'f', 4);//电压平均值
		//qDebug() << strAvg_U;

		QString strMax_P = QString::number(m_p[m_p.size() - 2], 'f', 4);//电阻率最大值
		QString strMin_P = QString::number(m_p[m_p.size() - 1], 'f', 4);//电阻率最小值
		QString strAvg_P = QString::number(m_p[m_p.size()], 'f', 4);//电阻率平均值
		...
		series2->clear();
		for (int i = 0; i < (m_p.size() - 3); i++)
		{
			//series2->append(i, find_P(voltage)[i]);//读取电阻率数据
			series2->append(i, m_p[i]);//读取电阻率数据
		}
	}
	else
	{
...
vector<double>  dianzulv::find_P(float voltage[])//初始200个电压数据得到电阻率处理函数
{
	...
	int start_up = 0, end_down = 0, number_U = 0;
	...
	number_U = end_down - start_up;

	//float p[number_U];//选取硅片经过探头下方时的电压信号数目计算得到的电阻率数目基础上加上3个（最大值、最小值、平均值）
	...
	float  max = p[0],min = p[0],mean_p=0.00;//初始最大最小电阻率值初始赋值
	...
	mean_p = sum_p / number_U;//平均数

	vector<double> pmax_min_mean;
	pmax_min_mean.push_back(max);
	pmax_min_mean.push_back(min);
	pmax_min_mean.push_back(mean_p);

	p.insert(p.end(), pmax_min_mean.begin(), pmax_min_mean.end());//电阻率数据区间与电阻率最大值、最小值、平均值进行数组拼接；
	return p;
}
	...
	: QMainWindow(parent)
{
	...
	axisX2->setRange(0,150);//设置X坐标轴范围
	//axisX2->setRange(0,200);//设置X坐标轴范围
	//axisX2->setRange(0, (ceil(m_p.size()/ 10)*100));//设置X坐标轴范围

	axisX2->setTitleText("扫描点数");//标题
	axisX2->setLabelFormat("%.2f"); //标签格式：每个单位保留0位小数

	////axisX2->setTickCount(ceil(m_p.size())/ 10 ); //主分隔个数：以10为基本单位进行区分
	//axisX2->setTickCount(ceil(m_p.size() / 25));
	axisX2->setTickCount(7);

	axisX2->setMinorTickCount(0); //每个单位之间绘制了0虚网线
	//axisX2->setGridLineVisible(false);//决定是否显示网格线
	axisX2->setGridLineVisible(true);//决定是否显示网格线
	...
	axisY2->setGridLineVisible(true);//决定是否显示网格线
