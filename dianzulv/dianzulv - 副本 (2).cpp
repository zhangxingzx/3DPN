...
////vector<int> dianzulv::Line(float voltage[]) //电阻率拟合直线斜率求上升沿和下降沿
//int dianzulv::Line(float voltage[], int flag) //电阻率拟合直线斜率求上升沿和下降沿
int dianzulv::linepoint(float voltage[], int flag) //电阻率拟合直线斜率求上升沿和下降沿
{
	if (flag = 1)
	{
		//计算选择电压信号的上升沿和下降沿位置
		...
		int point_U = 0;//上升沿位置，下降沿位置；
		float km = 0.02, kn = -0.02;//上升沿斜率，下降沿斜率；
		int MM = 0;
		int NN = 0;
		...
		for (int i = 0; i < 200; )
		{
			...
			{
				{
					vecProfile.push_back(float(voltage[i + 2]));
				}
				i++;
			}
			point_U = i;
			double k = 0;
			LineFit_Gradient(vecProfile, k);//计算扫描框数组电压数据斜率

			/*if (flag == 1)
			{
				if ((abs(k) > km) && (k > 0) && (k < 0.3))
				{
					point_U = point_U;
					MM = MM + 1;
				}
		...
		//vecNum_point[0] = start_up;     //上升沿
	    //vecNum_point[1] = end_down;     //下降沿

		vecNum_point[0] = 90;     //上升沿
		vecNum_point[1] = 180;     //下降沿
		//vector<int> res;
		//res.push_back(100);
		//res.push_back(180);
		//return res;
...
float* dianzulv::find_P(float voltage[])//初始200个电压数据得到电阻率处理函数
{
	...
	/*int start_up = linepoint(voltage, 1)[0];
	int end_down = linepoint( voltage,1)[1];*/

	cout << "123456789" << endl;
	//int start_up = linepoint(voltage, 1);     //上升沿
	//int end_down = 180;     //下降沿
